...
export default class UserController {
  ...
  public static async createUser(ctx: BaseContext): Promise<void> {
    ...
    userToBeSaved.password = crypto
      .createHmac("sha256", ctx.request.body.password)
      .digest("hex");
    ...
    if (errors.length > 0) {
      ...
      await (ctx.request.body.password !== ctx.request.body.passwordConfirm)
    ) {
      // return BAD REQUEST status code and password does not matched error
      ctx.status = 400;
      ctx.body = "The specified password does not matched";
